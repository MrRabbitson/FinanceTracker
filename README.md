...
- `ai`: конфигурация Hugging Face. Нужен действующий `hf_api_token` с доступом к выбранной модели. По умолчанию уже есть, однако он публичный, и бесплатный период может закончится. Получить токен: [клик](https://huggingface.co/settings/tokens)
